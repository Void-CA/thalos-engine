...
fn zero_config_ee_at_origin() {
    // Z is vertical. Spherical polar RRP: R(z), R(y), P(x).
...
fn azimuth_rotates_in_xy() {
    // R(z) rotates arm in XY plane (horizontal).
...
fn polar_tilt_moves_ee_in_xz() {
    // R(y) tilts arm in XZ plane (vertical plane).
